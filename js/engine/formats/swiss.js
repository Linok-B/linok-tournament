...
export async function advanceStage(stageData, config, allPlayers) {
    ...
    playersToPair.forEach((p, idx) => playerRankMap.set(p.id, idx));

    // Sort nextRoundMatches by Board/Table Number:
    nextRoundMatches.sort((mA, mB) => {
        // 1. Byes always go to the bottom board
        if (mA.isBye && !mB.isBye) return 1;
        if (!mA.isBye && mB.isBye) return -1;
        if (mA.isBye && mB.isBye) return 0;

        // 2. Best player on board (lower rank index is better)
